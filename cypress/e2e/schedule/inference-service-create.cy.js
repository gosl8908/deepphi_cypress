...
describe('Inference Service Create', () => {
    ...
    afterEach('Status Fail', () => {
        ...
        cy.get('h2').contains('마이 인퍼런스', { timeout: 10 * 1000 });
        /* 인퍼런스 삭제 */
        cy.get('body')
            .invoke('text')
            .then(text => {
                ...
                cy.get('tr.ng-star-inserted > :nth-child(4)')
                    ...
                    .then(Title => {
                        ...
                        /* 평가 프로젝트 삭제 */
                        cy.get('.left-nav__container').contains('마이 프로젝트').invoke('click');
                        cy.contains('프로젝트 활동 내역', { timeout: 20 * 1000 });
                        /* 프로젝트 검색 */
                        cy.get('.search-box > .input-form').type(Title);
                        cy.get('.search-box > .btn-primary').invoke('click');
                        cy.wait(3000);
                        cy.get('.title').invoke('click');
    ...
    after('Send Email', () => {
        ...
        emailModule.Email({
            TestFails: TestFails,
            EmailTitle: `[${Cypress.env('EmailTitle')}][Prod] Inference Service Create`,
            TestRange: TestRange,
