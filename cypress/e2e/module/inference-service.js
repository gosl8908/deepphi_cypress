...
function InferenceRun() {
  ...
  cy.get(':nth-child(2) > .table-layout > tbody > :nth-child(4) > td').then(status => {
      const A = status.text().includes('대기');
      const B = status.text().includes('중지');
      if (A || B) {
          cy.get('.btn-primary').click();
          cy.get(':nth-child(2) > .table-layout > tbody > :nth-child(4) > td').contains('실행', {timeout : 30*1000});
          cy.log('인퍼런스 서비스 - 실행완료');
          cy.wait(3*1000);

      } else {
          cy.log('에러 - 실행가능한 서비스 없음');
      }
  });
