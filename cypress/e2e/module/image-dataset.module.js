function imagedataset(ImageDatasetName, date_label) {
  ...
  cy.get(
    "form.ng-dirty > .create-dataset > .step-content-box > .page-button > .btn"
  ).click(); // 다음
  cy.get(
    ":nth-child(1) > dd > ul > :nth-child(1) > .radio-graphic > div > label > em"
  ).click(); // 2D 이미지 데이터 선택
  cy.get(
    ":nth-child(2) > dd > ul > :nth-child(1) > .radio-graphic > div > label > em"
  ).click(); // 분류 선택
  cy.get(
    ":nth-child(1) > .folder-structure-content > .folder-structure-content__header"
  ).click(); // 유형 1 선택
  cy.get(
    "form.ng-untouched > .create-dataset > .step-content-box > .page-button > .btn-primary"
  ).click({ force: true }); // 다음
  cy.wait(3000);

  /* 파일 업로드 */
  cy.fixture("image/2D_CL_Case1/Garbage Classification (2D Classification Case1).zip")
  .then((fileContent) => {
    cy.get('input[accept=".zip"][type="file"]').attachFile({
      fileContent,
      filePath: "C:/my-cypress-project/cypress/fixtures/image/2D_CL_Case1/Garbage Classification (2D Classification Case1).zip",
  ...
  cy.get(
    ".right-content > :nth-child(1) > .list-dropdown > :nth-child(1) > button"
  ).click(); // 검색된 모든 파일
