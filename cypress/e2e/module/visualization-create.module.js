function visualizationCreate(module, type) {
  ...
  cy.get('.tag__record-graph--graph').then(text => {
    const type = text.text().includes('빈도 그래프(Count Plot)');
    if (type) {
...
cy.get('.tag__record-graph--graph').then(text => {
  const type = text.text().includes('히스토그램(Histogram)');
  if (type) {
    cy.get('.ui-dropdown-trigger').click();
...
cy.get('.tag__record-graph--graph').then(text => {
  const type = text.text().includes('산점도(Scatter Plot)');
  if (type) {
...
cy.get('.tag__record-graph--graph').then(text => {
  const type = text.text().includes('밀도분포(Density Plot)');
  if (type) {
    cy.get(':nth-child(2) > dd > .input-form > .ui-dropdown').click();
    cy.get(':nth-child(2) > .ui-dropdown-item').click();
    cy.get(':nth-child(3) > dd > .input-form').type('100');
...
cy.get('.tag__record-graph--graph').then(text => {
  const type = text.text().includes('히트맵(Heat Map)');
  if (type) {
    cy.get(':nth-child(2) > dd > .input-form > .ui-dropdown').click();
    cy.get(':nth-child(1) > .ui-dropdown-item').click();
    cy.get(':nth-child(3) > dd > .input-form > .ui-dropdown').click();
...
cy.get('.tag__record-graph--graph').then(text => {
  const type = text.text().includes('상자 수염 그림(Box Plot)');
  if (type) {
    cy.get(':nth-child(2) > dd > .input-form > .ui-dropdown').click();
    cy.get(':nth-child(1) > .ui-dropdown-item').click();
    cy.get(':nth-child(3) > dd > .input-form > .ui-dropdown').click();
