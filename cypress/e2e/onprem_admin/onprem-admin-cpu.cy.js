...
describe('Module Run Test', () => {
    ...
    it('CPU Management Search', () => {
        cy.get('.nav-button > button').click();
        cy.contains('CPU 관리').click();
        cy.wait(5 * 1000);
        ...
        cy.get('.search-box__list > li:nth-child(5)').type('36338');
        cy.get('.search-box__list > li:nth-child(6)').type('img240109');
        cy.get('.search-box__list > li:nth-child(7)').type('Resize');
        cy.get('.search-box__list > li:nth-child(8)').type(Cypress.env('OnpremAdminId'));
        cy.get('.search-box__list > li:nth-child(9)').type('onprem240109@ruu.kr');

        cy.contains('검색').click();
        cy.wait(3 * 1000);

        cy.get('.p-datatable-tbody > tr:nth-child(1) > td:nth-child(2)').contains('1');
        cy.get('.p-datatable-tbody > tr:nth-child(1) > td:nth-child(3)').contains('36338');
        cy.get('.p-datatable-tbody > tr:nth-child(1) > td:nth-child(4)').contains('img240109');
        cy.get('.p-datatable-tbody > tr:nth-child(1) > td:nth-child(1)').contains('TRAIN');
        cy.get('.p-datatable-tbody > tr:nth-child(1) > td:nth-child(2)').contains('Resize');
        ...
        cy.get('.p-datatable-tbody > tr:nth-child(1) > td:nth-child(7)').contains('00:00:34');
        cy.get('.p-datatable-tbody > tr:nth-child(1) > td:nth-child(8)').contains('00:00:34');
        cy.get('.p-datatable-tbody > tr:nth-child(1) > td:nth-child(9)').contains('396 / 0.74 GB');
    ...
    after('Send Email', () => {
        const TestRange = '1. CPU 관리 검색';

        emailModule.Email({
            TestFails: TestFails,
            EmailTitle: `[${Cypress.env('EmailTitle')}][Onprem][Admin] CPU Management`,
            TestRange: TestRange,
