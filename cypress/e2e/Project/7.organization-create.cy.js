...
describe("Organization Create", () => {
  ...
  it("Organization Create", () => {
    ...
    // 그룹 생성
    cy.get(
      ":nth-child(2) > .left-navigation--sub-navi > :nth-child(1) > button > span"
    ).click(); // 그룹 관리
  ...
  it("Organization Credit Charge", () => {
    ...
    cy.get(
      ":nth-child(1) > .text-left > .display-block > .p-dropdown > .p-dropdown-label"
    ).click(); // 크레딧 타입
    cy.get(":nth-child(1) > .p-dropdown-item").click(); // 유료
    cy.get(
      ":nth-child(2) > .text-left > .display-block > .p-dropdown > .p-dropdown-label"
    ).click(); // 사유
    cy.get(":nth-child(1) > .p-dropdown-item").click(); // 리워드 적립
    cy.get(".text-right > .display-block").type("100000"); // 금액 입력
    cy.get(
      ".wrap--input-and-button > .p-calendar__width__custom > .p-calendar > .p-inputtext"
    ).click(); // 유효기간
    ...
    cy.get(
      "admin-credit-alert-modal.ng-star-inserted > .modal--wrap > .modal--btn-area > .btn"
    ).click(); // 확인
    cy.wait(3000);
  });
  ...
  it("Organization DISK subscribe", () => {
    ...
    emailModule.email(
      ...
      Cypress.env("Organization_Create_emailbody")
    );
  });
});
