const { loginModule, createModule, ApiModule, EmailModule, InferenceServiceModule, ConstantModule: c } = require('../module/manager.module.js');
describe('로그인', () => {
  ...
  it('test', function () {
    ...
    // cy.get('.modeler__status').contains('실행', { timeout: 10*1000 }).should('be.visible');
    // cy.get('.modeler__status').contains('완료', { timeout: 360*1000 }).should('be.visible');
    // cy.wait(3*1000);

    cy.visit('https://modeler.deepphi.ai/modeler/39883?testId=39901&loadedProjectPage=1');
    // cy.visit('https://modeler.deepphi.ai/modeler/39885?testId=39898&loadedProjectPage=1');
    ...
    cy.get('[style="width: calc(100% - 76px);word-break: break-all"]').then(($el) => {
      ...
      return $el.text();
        ...
        const endPoint = `${$e2}/api/prediction`;
      ...
      //   // 텍스트 추출
      //   const endpoint2 = $el.text();
  
      //   Cypress.env('endpointText', endpoint2);
      //   cy.log('확인된 endpointText 값:', Cypress.env('endpointText'));
      //   });

      //   cy.log('aaa:'+Cypress.env('endpointText'))

      //   const formData = new FormData(); // FormData 생성
      //   const endPoint = `${Cypress.env('endpointText')}/api/prediction`;
      //           cy.log(`endPoint : ${endPoint}`);
      //           cy.fixture('image/2D_CL_Case1/google_0001.jpg', 'binary')
      //               .then(Cypress.Blob.binaryStringToBlob)
      //               .then(fileContent => {
